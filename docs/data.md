...
| Progression de la visite guidée | Clic sur « 🚀 Commencer » ou « Passer l'introduction » | `localStorage` du navigateur (clé `onboarding_completed`) | Aucune (jamais envoyée au serveur) | Ne pas réafficher la visite guidée à chaque visite — voir [Guide → Visite guidée](guide.md#visite-guidée-onboarding) |
...
Aucune base de données externe ni `IndexedDB` n'est utilisé : l'essentiel du stockage se fait côté serveur, dans des fichiers JSON locaux (voir [`src/utils/cache.py`](../src/utils/cache.py) et [`src/utils/favorites.py`](../src/utils/favorites.py)). Deux exceptions, purement côté navigateur, dans `localStorage` : l'acceptation de l'avertissement légal (clé `legal_notice_acknowledged`) et l'état de la visite guidée (clé `onboarding_completed`) — voir [Informations légales](legal.md) et [Guide → Visite guidée](guide.md#visite-guidée-onboarding). Rien d'autre n'y est stocké.
