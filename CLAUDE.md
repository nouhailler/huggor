...
Contenu centralisé dans [`src/onboarding.py`](src/onboarding.py) (5 étapes), accessible **uniquement à la demande** depuis l'écran « ⚙️ Paramètres » (bouton « 🧭 Revoir la visite guidée », géré par `settings_onboarding_button`/`_SETTINGS_TO_ONBOARDING_JS`). Réutilise le même mécanisme d'overlay (`.hf-legal-overlay`/`.hf-legal-card`, généré via `_overlay_switch_js`) que les mentions légales et l'écran « À propos » — voir le commentaire au-dessus de `.hf-legal-overlay` dans `app.py`. Détails complets : [`docs/guide.md`](docs/guide.md#visite-guidée-onboarding).

- **Ne plus jamais la déclencher automatiquement au premier lancement** : elle s'affichait initialement juste après l'acceptation de l'avertissement légal (`_LEGAL_ACCEPT_THEN_MAYBE_ONBOARDING_JS`/`_STARTUP_CHECK_JS`, retirés). Un chargement lent (Render en sortie de veille, notamment) laissait le bouton « Suivant » visuellement figé sans aucun retour visible (`show_progress="hidden"`), perçu comme un bouton cassé — corrigé en supprimant l'affichage automatique plutôt qu'en creusant plus loin la latence. Ne pas réintroduire ce déclenchement automatique sans revoir aussi le retour visuel pendant un chargement lent.
- **Stockage** : `localStorage` du navigateur, clé `onboarding_completed` (+ `onboarding_completed_version`), indépendante de `legal_notice_acknowledged`. 100 % côté client. Toujours écrite quand l'utilisateur termine ou passe la visite depuis Paramètres, même si elle ne sert plus à en conditionner l'affichage automatique.
...
- **Tester** : menu ☰ → « ⚙️ Paramètres » → « 🧭 Revoir la visite guidée ».
